...
FString FScriptCodeGeneratorBase::GetPropertyTypeCPP(FProperty *Property, uint32 PortFlags /*= 0*/)
{
	...
	static const FString TObjectPtrDecl(TEXT("TObjectPtr<"));

	// PropertyClass->GetPrefixCPP(), *PropertyClass->GetName()
	...
	else if (PropertyType.StartsWith(TObjectPtrDecl, ESearchCase::CaseSensitive))
	{
		int32 FirstSpaceIndex = PropertyType.Find(TEXT(" "), ESearchCase::CaseSensitive);
		PropertyType = PropertyType.Mid(FirstSpaceIndex + 1) + TEXT("*");
	}
	return PropertyType;
}

FString FScriptCodeGeneratorBase::GenerateFunctionDispatch(UFunction *Function, bool bHasReturn)
{
	FString Params;

	const bool bHasParamsOrReturnValue = bHasReturn || (Function->Children != nullptr);
