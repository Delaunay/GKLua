	...
	if (!(Param->GetPropertyFlags() & CPF_ReturnParm))
	{
		...
		else if (Param->IsA(FStructProperty::StaticClass()))
		{
			...
			else if (StructProp->Struct->GetFName() == Name_Color)
			{
				...
				// UE5: explicit FColor(const FLinearColor& LinearColor); is private
				return FString::Printf(TEXT("%s(InScriptContext, %d)).ToFColor(false)"), *Initializer, ParamIndex);
			}
		...
		{
			// *GetPropertyTypeCPP(Param, CPPF_ArgumentOrReturnValue) == TObjectPtr<UHLODLayer>
			// which breaks, because casting from void* is not supported
...
FString FLuaScriptCodeGenerator::ExportFunction(const FString &ClassNameCPP, UClass *Class, UFunction *Function)
{
	...
	if (FuncSuper == NULL)
	{
		...
		FunctionBody += GenerateFunctionDispatch(Function, ReturnValue != nullptr);
...
void FLuaScriptCodeGenerator::GlueAllGeneratedFiles()
{
	...
	// This helper is defined inside Script Plugin
	LibGlue += FString::Printf(TEXT("#include \"UObject/ObjectPtr.h\"\r\n"));
	LibGlue += FString::Printf(TEXT("extern FProperty* FindScriptPropertyHelper(UClass* Class, FName PropertyName);\r\n"));
	// LibGlue += FString::Printf(TEXT("#include \"LuaIntegration.h\"\r\n"));

	/*
	LibGlue += FString::Printf(TEXT(
		"extern \"C\"\n"
		"{\n"
