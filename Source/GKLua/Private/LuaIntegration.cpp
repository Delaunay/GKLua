...
// There is an issue where this cannot not have been generated yet
#if __has_include("GeneratedScriptLibraries.inl")
...
bool FLuaUtils::CallFunction(lua_State *LuaState, const ANSICHAR *FunctionName)
{
	...
	{
		UE_LOG(LogGKLua, Warning, TEXT("Cannot call Lua function %s: %s"), ANSI_TO_TCHAR(FunctionName), ANSI_TO_TCHAR(lua_tostring(LuaState, -1)));
		bResult = false;
	}
	return bResult;
}
...
static int32 LuaUnrealLog(lua_State *LuaState)
{
	...
	UE_LOG(LogGKLua, Log, TEXT("%s"), *Message);
...
{
	UE_LOG(LogGKLua, Error, TEXT("PANIC: unprotected error in call to Lua API(%s)"), ANSI_TO_TCHAR(lua_tostring(lua_State, -1)));
	return 0;
}
...
FLuaContext::FLuaContext()
	: bHasTick(false), bHasDestroy(false), bHasBeginPlay(false), LuaState(NULL)
{
}
...
bool FLuaContext::Initialize(const FString &Code, UObject *Owner)
{
	...
	{
		UE_LOG(LogGKLua, Warning, TEXT("Cannot load Lua script for %s: %s"), *Owner->GetPathName(), ANSI_TO_TCHAR(lua_tostring(LuaState, -1)));
	}
	return bResult;
}
...
bool FLuaContext::CallFunction(const FString &FunctionName)
{
	...
	{
		UE_LOG(LogGKLua, Warning, TEXT("Failed to call function '%s' "), *FunctionName);
	}
...
bool FLuaContext::SetFloatProperty(const FString &PropertyName, float NewValue)
{
	...
	{
		UE_LOG(LogGKLua, Warning, TEXT(" [LUA] Global '%s' is NaN. "), *PropertyName);
	}
...
bool FLuaContext::SetStringProperty(const FString &PropertyName, const FString &NewValue)
{
	...
	UE_CLOG(!bExists, LogGKLua, Warning, TEXT(" [LUA] Global '%s' is not a String. "), *PropertyName);
	return bExists;
}
...
FString FLuaContext::GetStringProperty(const FString &PropertyName)
{
	...
	{
		UE_LOG(LogGKLua, Warning, TEXT(" [LUA] Global '%s' is not a String. "), *PropertyName);
	}
...
void FLuaContext::GetScriptDefinedFields(TArray<FScriptField> &OutFields)
{
	...
	while (lua_next(LuaState, -2) != 0)
	{
		...
		{
		case LUA_TNUMBER:
			PropertyInfo.PropertyClass = FFloatProperty::StaticClass();
			break;
		case LUA_TSTRING:
			PropertyInfo.PropertyClass = FStrProperty::StaticClass();
			break;
		case LUA_TBOOLEAN:
			PropertyInfo.PropertyClass = FBoolProperty::StaticClass();
			break;
		case LUA_TNIL:
		case LUA_TUSERDATA:
			PropertyInfo.PropertyClass = FObjectProperty::StaticClass();
			break;
		case LUA_TFUNCTION:
		...
		if (PropertyInfo.Class || PropertyInfo.PropertyClass)
		{
